...
REPOSITORY_LOGGED_IN_FILTER = lambda u: Q(
    Q(Q(is_public=True) | Q(member__user=None) | Q(member__user=u)) & Q(is_enabled=True))
...
class RepositoryManager(models.Manager):
    def visible_repositories_for_user(self, user=None):
        ...
        return self.get_queryset().filter(qset)
    ...
    def sync_with_fs(self):
        ...
        for entrie in entries:
            ...
            try:
                ...
                if not (
                    os.path.isdir(os.path.join(repository_path, '.git')) or repository_path.endswith(".git")): continue
        ...
        for entrie in entries:
            repository_path = os.path.join(settings.GIT_REPOSITORY, entrie)
            if (os.path.isdir(os.path.join(repository_path, '.git')) or repository_path.endswith(".git")):
                local_path.append(repository_path)
        respository_path = Repository.objects.values_list("path")
        respository_path = [i[0].encode('utf8') for i in respository_path]
        result['outdated'] = [i for i in respository_path if i not in local_path ]
        for i in result['outdated']:
            logging.warning("the repository '%s' is not available on the filesystem" % i)

        # # In the database but not on the filesystem, we log it
        # for repo_path in [repo_path for repo_path in repositories if not (
        #     os.path.isdir(os.path.join(repository_path, '.git')) or repository_path.endswith(".git"))]:
...
class Repository(models.Model):
    ...
    def recent_commits(self):
        return self.repo().iter_commits(max_count=getattr(settings, 'GITHUB_RECENT_COMMITS_COUNT', 10))
        # return self.repo().commits(max_count=getattr(settings, 'GITHUB_RECENT_COMMITS_COUNT', 10))   //'Repo' object has no attribute 'commits'

    @property
    def last_commit(self):
        return self.repo().commit()
        # return self.repo().commits(max_count=1)[0]           //AttributeError: 'Repo' object has no attribute 'commits'
